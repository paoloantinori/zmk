...
static void set_battery_symbol(lv_obj_t *icon, struct battery_status_state state) {
    ...
    if (usb_present != usb_prev || stage != stage_prev) {
        switch (stage) {
        case 5:
            lv_image_set_src(icon, usb_present ? &batt_100_chg : &batt_100);
            break;
        case 4:
            lv_image_set_src(icon, usb_present ? &batt_75_chg : &batt_75);
            break;
        case 3:
            lv_image_set_src(icon, usb_present ? &batt_50_chg : &batt_50);
            break;
        case 2:
            lv_image_set_src(icon, usb_present ? &batt_25_chg : &batt_25);
            break;
        case 1:
            lv_image_set_src(icon, usb_present ? &batt_5_chg : &batt_5);
            break;
        default:
            lv_image_set_src(icon, usb_present ? &batt_0_chg : &batt_0);
            break;
        }
